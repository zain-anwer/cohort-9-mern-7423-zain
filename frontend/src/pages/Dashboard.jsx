...
export const Dashboard = () => { 
    ...
    const initProfileSocketListeners = useProfileStore((state) => state.initSocketListeners)
    const cleanProfileSocketListeners = useProfileStore((state) => state.cleanSocketListeners)

    useEffect(() => {
        const initialize = async () => {
            try {
                await fetchUser()
                initProfileSocketListeners()
            }
            catch(error) {
                ...
                > 
                    <div className="sticky top-0 z-10 flex items-center gap-4 border-b border-stone-300 bg-stone-50 px-4 py-3 sm:px-6 sm:py-4 lg:px-8"> 
                        ...
                        <div className="ml-auto flex items-center gap-4"> 
                            ...
                            <button type='button' onClick={handleProfile} className="flex items-center gap-2 rounded-md p-2 text-sm font-medium text-gray-600 hover:bg-gray-100 hover:text-gray-900"> 
                                {user ? ( 
                                    <> 
                                        <img 
